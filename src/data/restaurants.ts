import { classicBurger, vegeterianPizza, vegyBurger } from '../assets';
import rest_1 from '../assets/images/restaurants/1.jpg';
import rest_2 from '../assets/images/restaurants/2.jpg';
import rest_3 from '../assets/images/restaurants/3.jpg';
import rest_4 from '../assets/images/restaurants/4.jpg';
import rest_5 from '../assets/images/restaurants/5.jpg';
import { menu } from './meals';
const restaurants = [
  {
    id: 1,
    name: 'Le Petit Bistro',
    location: 'Rue de la Liberté, Tunis',
    img: classicBurger,
    rating: 4,
    description: 'A cozy bistro offering a delightful dining experience.',
    workingTime: '8AM - 10PM',
    menu: menu,
  },
  {
    id: 2,
    name: 'La Maison des Saveurs',
    location: 'Avenue Habib Bourguiba, Sousse',
    img: vegeterianPizza,
    rating: 5,
    description: 'Experience a gastronomic journey with a wide range of flavors.',
    workingTime: '8AM - 10PM',
    menu: menu,
  },
  {
    id: 3,
    name: 'Le Jardin Enchanté',
    location: 'Rue du Lac, Tunis',
    img: rest_3,
    rating: 4,
    description: 'Discover a magical garden setting with exquisite cuisine.',
    workingTime: '8AM - 10PM',
    menu: menu,
  },
  {
    id: 4,
    name: "L'Oasis Bleue",
    location: "Boulevard de l'Environnement, Hammamet",
    img: rest_4,
    rating: 2,
    description: 'Escape to a tranquil oasis serving a blend of international and local dishes.',
    workingTime: '8AM - 10PM',
    menu: menu,
  },
  {
    id: 5,
    name: 'La Merienda',
    location: 'Avenue Mohamed V, Tunis',
    img: rest_5,
    rating: 3,
    description: 'Indulge in a relaxed ambiance while savoring delicious snacks and drinks.',
    workingTime: '8AM - 10PM',
    menu: menu,
  },
];

export default restaurants;
